...
# ==========================================================
# 1. Setup Models and Video Source
# ==========================================================
root = tk.Tk()
root.withdraw()

MODELS_DIR = r"C:\Users\NOURA\Downloads\Models"
DOWNLOADS_DIR = r"C:\Users\NOURA\Downloads"

phone_path = (
    os.path.join(MODELS_DIR, "bestphonenew.pt")
    if os.path.exists(os.path.join(MODELS_DIR, "bestphonenew.pt"))
    else os.path.join(DOWNLOADS_DIR, "bestphonenew.pt")
)
belt_path = (
    os.path.join(MODELS_DIR, "lastone.pt")
    if os.path.exists(os.path.join(MODELS_DIR, "lastone.pt"))
    else os.path.join(DOWNLOADS_DIR, "lastone.pt")
)

print("Please select a video file for analysis...")
video_source = filedialog.askopenfilename(
    title="Select Driver Video for Testing",
    filetypes=[("Video files", "*.mp4 *.avi *.mov *.mkv")],
)

# Switch to webcam if no file is selected
if not video_source:
  print("No file selected. Opening webcam (camera 0)...")
  video_source = 0
...
CURRENT_TRIP_ID = f"TRIP_{timestamp_suffix}"
csv_save_path = (
    rf"C:\Users\NOURA\Downloads\unified_dms_violations_{timestamp_suffix}.csv"
)


# ==========================================================
# 3. Text-to-Speech Engine and Helper Functions
# ==========================================================
...
def create_dashboard_panel(width=380, height=480):
  ...
  score_color = (
      (0, 255, 120)
      if safety_score >= 80
      else ((0, 165, 255) if safety_score >= 50 else (0, 0, 255))
  )
  cv2.putText(
      panel,
      f"SAFETY SCORE: {max(0, safety_score)}%",
      (20, 60),
      cv2.FONT_HERSHEY_DUPLEX,
      0.6,
      score_color,
...
# ==========================================================
# 4. Video Stream and Real-Time Inference
# ==========================================================
...
while cap.isOpened():
  ...
  # A. Facial landmarks with MediaPipe
  ...
  # B. Phone detection with spatial filtering
  phone_results = phone_model.predict(source=frame, conf=0.45, verbose=False)
  has_phone = False
  if phone_results[0].boxes is not None and len(phone_results[0].boxes) > 0:
    for box, conf, cls in zip(
        phone_results[0].boxes.xyxy,
        phone_results[0].boxes.conf,
        phone_results[0].boxes.cls,
    ):
      ...
      if "phone" in cls_name or "cell" in cls_name or int(cls) == 0:
        ...
        # Filter out false positives in upper frame or oversized boxes
        if y1 > (h * 0.22) and cy > (h * 0.25) and bw < 250 and bh < 250:
          has_phone = True
  ...
  # C. Seatbelt detection with geometric filtering
  belt_results = belt_model.predict(source=frame, conf=0.35, verbose=False)
  has_belt = False

  # Check standard bounding boxes
  if belt_results[0].boxes is not None and len(belt_results[0].boxes) > 0:
    for box, conf, cls in zip(
        belt_results[0].boxes.xyxy,
        belt_results[0].boxes.conf,
        belt_results[0].boxes.cls,
    ):
      ...
      if "no" not in cls_name:
        ...
        if aspect_ratio < 0.85 and cy > (chin_y * 0.70):
          has_belt = True
  ...
  # Check oriented bounding boxes (OBB)
  if (
      not has_belt
      and belt_results[0].obb is not None
      and len(belt_results[0].obb) > 0
  ):
    for obb_box, conf in zip(
        belt_results[0].obb.xyxyxyxy, belt_results[0].obb.conf
    ):
  ...
  if has_phone:
    ...
    if (current_time - last_phone_time) > PHONE_COOLDOWN_SEC:
      ...
      safety_score -= 10
      recent_logs.append(
          {"time": display_time_str, "type": "Phone Violation"}
      )
  ...
  # 2. Seatbelt Alert Logic (3-second buffer before logging)
  if not has_belt:
    if unbuckled_start_time is None:
      unbuckled_start_time = current_time
    elif (current_time - unbuckled_start_time >= 3.0) and not belt_logged:
      total_belt += 1
      safety_score -= 15
  ...
  # 3. Drowsiness and Yawning Logic
  if face_detected:
    ...
    if ear < EAR_THRESHOLD:
      ...
      if counter_ear >= EAR_CONSEC_FRAMES:
        ...
        if (current_time - last_drowsy_time) > DROWSY_COOLDOWN_SEC:
          ...
          safety_score -= 20
          recent_logs.append(
              {"time": display_time_str, "type": "Drowsiness Alert"}
          )
          violations_history.append({
              "trip_id": CURRENT_TRIP_ID,
    ...
    if mar > MAR_THRESHOLD:
      ...
      if counter_mar >= MAR_CONSEC_FRAMES:
        ...
        if (current_time - last_yawn_time) > YAWN_COOLDOWN_SEC:
          ...
          safety_score -= 5
          recent_logs.append(
              {"time": display_time_str, "type": "Yawn Detected"}
          )
          violations_history.append({
              "trip_id": CURRENT_TRIP_ID,
  ...
  # Combine video display with the telemetry side dashboard
...
# ==========================================================
# 5. Export Violations Log CSV
# ==========================================================
if violations_history:
  df = pd.DataFrame(violations_history)
  df = df[["trip_id", "violation_type", "timestamp"]]
  df.to_csv(csv_save_path, index=False)
  print(f"\n[+] Violation log saved successfully to: {csv_save_path}")
else:
  print(
      "\n"
      "[!] Perfect Safe Driving: No violations logged (Safety Score: 100%)."
  )